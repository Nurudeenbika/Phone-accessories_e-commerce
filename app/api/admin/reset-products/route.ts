  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: `Successfully created ${newProducts.length} products!`,
        phones: newProducts.filter((p) => p.category === "phone").length,
        accessories: newProducts.filter(
          (p) => p.category === "phone-accessories",
        ).length,
      });
    } catch (error) {
      throw error;
