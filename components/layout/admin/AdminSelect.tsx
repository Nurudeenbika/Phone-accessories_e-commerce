  ...
  return (
    <div ref={selectRef} className={`relative ${className}`}>
      ...
      <button
        ...
        className={`w-full px-4 py-2 text-left bg-white border border-gray-200 rounded-lg hover:border-primary-600 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-600 transition-colors ${
          isOpen ? "border-primary-600 ring-2 ring-primary-500" : ""
        }`}
      >
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 shadow-lg max-h-60 overflow-auto">
          {options.map((option) => (
            ...
            >
              ...
              {option.value === value && (
                <MdCheck className="w-4 h-4 text-primary-600" />
              )}
            </button>
          ))}
        </div>
